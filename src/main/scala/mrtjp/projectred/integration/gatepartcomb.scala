...
:
    ...
    def cycleShape(shape:Int):Int =
        ...
        var shape1 = shape
        import java.lang.Integer.{bitCount, numberOfLeadingZeros as lead}
...
:
    ...
    override def calcOutput(gate:ComboGatePart, input:Int) = if input == 0 then 0 else 1
...
:
    ...
    override def getOutput(gate:ComboGatePart, r:Int) = if r == 2 then gate.state>>4 else 0
...
:
    ...
    override def outputMask(shape:Int) = if shape == 0 then 11 else 9
    override def inputMask(shape:Int) = 4
