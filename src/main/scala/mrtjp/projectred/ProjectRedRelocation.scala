...
object RelocationConfig extends ModConfig("projectred-relocation")
{
    ...
    override protected def initValues(): Unit =
        ...
        moverMap = movers.put("mover registry", MovingTileRegistry.parseAndSetMovers(moverMap.toIndexedSeq), force = true)

        val sets = BaseCategory("Latched Sets", buildLatchSetsDesc)
        setMap = sets.put("latch registry", setMap)
        setMap = sets.put("latch registry", StickRegistry.parseAndAddLatchSets(setMap.toIndexedSeq), force = true)
