...
:
    ...
    override def update(): Unit =
        ...
        if real == null || side < 0 then return
        ...
        for i <- 0 until stock.getSizeInventory do breakable
          :
            ...
            val stockToKeep = if requestMode == 2 then Int.MaxValue else filt.getItemCount(keyStack.key)
            ...
            if toRequest <= 0 || (requestMode == 1 && inInventory > 0) then break()
