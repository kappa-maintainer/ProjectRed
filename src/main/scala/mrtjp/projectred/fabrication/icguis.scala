...
:
    var size: Size = Size.zeroSize
    override def frame: Rect = Rect(position, size)

    var lineColor: Int = EnumColour.LIME.argb(0xAA)
    var linePointerCalc: () => Point = () => Point.zeroPoint
...
:
    ...
    def getLogic: ITimerGuiLogic = gate.getLogic[ITimerGuiLogic]
...
:
    ...
    def getLogic: ICounterGuiLogic = gate.getLogic[ICounterGuiLogic]
