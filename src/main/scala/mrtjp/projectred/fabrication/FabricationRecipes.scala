...
class ICGateRecipe extends TBaseRecipe:
    override def getRecipeOutput: ItemStack = GateDefinition.ICGate.makeStack
...
trait TBaseRecipe extends IForgeRegistryEntry.Impl[IRecipe] with IRecipe:
    ...
    override def canFit(width: Int, height: Int): Boolean = width * height >= getRecipeSize
    override def matches(inv: InventoryCrafting, worldIn: World): Boolean = !getCraftingResult(inv).isEmpty
